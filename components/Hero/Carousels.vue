  ...
  <div class="carousel">
    ...
    <div class="list">
      <div v-for="(item, index) in items" :key="index" class="item">
        <img :src="useImg(item.image)" class="relative" />
        <div class="content">
          <div
            class="w-[200px] cat-date md:w-[600px] mb-5 flex justify-between"
          >
            <span class="p-2 bg-primary rounded-2xl text-lg">
              {{ item.category.name }}</span
            >
            <span class="p-2 rounded-2xl text-lg text-gray-300">
              {{ $formatDate(item.date_created) }}
            </span>
          </div>
          <div class="title line-clamp-2">{{ item.title }}</div>
          <div
            class="w-[300px] aut-views md:w-[600px] mb-5 flex justify-between"
          >
            <div class="gap-x-3 flex items-center justify-center">
              <i class="fa-solid fa-user"></i>
              <span class="p-2 rounded-2xl text-lg">
                {{
            ...
            <div
              v-if="item.views"
              class="flex items-center text-sm md:text-lg justify-center"
            >
...
const useImg = (url: string): string => {
