<template>
  ...
  <section class="gradient-pp vh-100">
    <div class="relative lg:h-[60vh] w-full grid grid-cols-1 lg:grid-cols-2">
      <div class="flex items-center justify-center">
        <div class="pt-28 lg:pt-1">
          <h1 class="text-white text-3xl font-bold" data-aos="fade-down">
  ...
  <div class="container mt-10 mx-auto mb-5">
    <div v-if="loading" class="text-center text-white"><LoadingSearchArticle /></div>
    <div v-else-if="articles.length === 0" class="text-center text-black">
      <!-- No results found -->
...
const handleSearch = async () => {
  ...
  try {
    const response = await useApi<IResponse<IArticle[]>>(
      `/items/article?filter[status]=published&filter[title][_contains]=${searchQuery.value}&fields=title,date_created,slug,image,category.name,user_created.first_name,user_created.last_name,user_created.avatar,description`,
      { method: 'GET' }
    );
